...
export default async (req) => {
  ...
  try {
    ...
    const task =
      instructions[tool] || instructions['AI Song'];

    // Real cover-art generation
    if (
      tool === 'Cover Art' &&
      ...
      const image = await client.images.generate({
        ...
        prompt:
          `${prompt}. ${style || ''}. ` +
          'Create original professional album cover artwork. ' +
          'No logos, no copyrighted characters, no imitation of a living artist.',
        size: '1024x1024'
      });
      ...
      messages: [
        {
          role: 'system',
          content:
            `You are the creative AI inside G.C.D Studio. ` +
            `${task} ` +
            `Return clean production-ready text. ` +
            `The user's requested style is ${style || 'not specified'}.`
        },
        {
          role: 'user',
          content: prompt
